...
var currentPage = 1; // Variable globale pour stocker la page actuelle seulement sur rechargement direct du URL
var pageReload = false; // pageReload sera mis à true lorsqu'on recharge manuellement l'url
setTimeout(function () {
...
document.getElementById('search-input').addEventListener('input', function (event) {
  ...
  loadProducts(event.target.value, document.getElementById('sort').value, document.querySelector('input[name="prix-min"]').value, document.querySelector('input[name="prix-max"]').value, getSelectedTags()
  // currentPage
  );
});
...
document.querySelector('input[name="range-min"]').addEventListener('change', function () {
  loadProducts(document.getElementById('search-input').value, document.getElementById('sort').value, document.querySelector('input[name="prix-min"]').value, document.querySelector('input[name="prix-max"]').value, getSelectedTags(), pageReload ? currentPage : 1);
});
...
document.getElementById('sort').addEventListener('change', function (event) {
  ...
  loadProducts(document.getElementById('search-input').value, event.target.value, document.querySelector('input[name="prix-min"]').value, document.querySelector('input[name="prix-max"]').value, getSelectedTags()
  // currentPage
  );
});
...
document.addEventListener('click', function (event) {
  if (event.target.matches('#pagination a')) {
    ...
    loadProducts(document.getElementById('search-input').value, document.getElementById('sort').value, document.querySelector('input[name="prix-min"]').value, document.querySelector('input[name="prix-max"]').value, getSelectedTags(), page);
...
window.onload = function () {
  ...
  var prixMin = urlParams.get('prix-min');
  var prixMax = urlParams.get('prix-max');
  pageReload = true;
  ...
  var minInput = document.querySelector('input[name="prix-min"]');
  var maxInput = document.querySelector('input[name="prix-max"]');
  if (prixMin) {
...
var defaultMinPrice = document.querySelector('input[name="range-min"]').min;
var defaultMaxPrice = document.querySelector('input[name="range-max"]').max;

// FONCTION pour charger les bouteilles avec les critères de recherche, de tri, de pagination et de tags
function loadProducts(searchQuery, sortOption) {
  var minPrice = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : defaultMinPrice;
  var maxPrice = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : defaultMaxPrice;
  var selectedTags = arguments.length > 4 ? arguments[4] : undefined;
  ...
  if (numPrixMin !== defaultMinPrice) {
    url += "&prix-min=".concat(encodeURIComponent(numPrixMin));
  }
  if (numPrixMax !== defaultMaxPrice) {
    url += "&prix-max=".concat(encodeURIComponent(numPrixMax));
  }
...
function updateUrl(searchQuery, sortOption) {
  var minPrice = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : defaultMinPrice;
  var maxPrice = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : defaultMaxPrice;
  var tags = arguments.length > 4 ? arguments[4] : undefined;
  var page = arguments.length > 5 ? arguments[5] : undefined;
