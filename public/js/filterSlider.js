...
function updateRangeSlider(sliderGroup) {
  ...
  var minVal = parseFloat(rangeInputs[0].value);
  var maxVal = parseFloat(rangeInputs[1].value);
...
document.querySelectorAll('.form-range').forEach(function (sliderGroup) {
  ...
  numberInputs.forEach(function (input) {
    input.addEventListener('input', function () {
      var minVal = parseFloat(numberInputs[0].value);
      var maxVal = parseFloat(numberInputs[1].value);
