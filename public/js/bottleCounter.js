...
document.querySelectorAll('.card-bouteille-qt').forEach(function (counter) {
  ...
  var deleteForm = counter.querySelector('.form-delete');
  var bouteilleType;
  var deplacerButton;
  ...
  decrementButton.addEventListener('click', function () {
    var totalBouteilles = cardCount.querySelector('p').innerHTML.trim();
    totalBouteilles = totalBouteilles.slice(0, -13);
    var currentValue = parseInt(input.value, 10);
    var totalBouteillesValeur = parseInt(totalBouteilles, 10);
    if (currentValue > 0) {
  ...
  function _updateQuantity() {
    _updateQuantity = _asyncToGenerator( /*#__PURE__*/_regeneratorRuntime().mark(function _callee(newQuantity) {
      var id, location, url, response, data;
      return _regeneratorRuntime().wrap(function _callee$(_context) {
        while (1) switch (_context.prev = _context.next) {
          case 0:
            id = card.id;
            location = card.getAttribute('data-location');
            url = "/bouteilles-".concat(location, "-modifier/").concat(id);
            if (bouteilleType == 'personnalisees') {
              url = "/bouteilles-personnalisees-celliers-modifier/".concat(id);
            }
            _context.prev = 4;
            _context.next = 7;
            return fetch(url, {
